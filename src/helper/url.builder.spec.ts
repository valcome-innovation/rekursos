...
describe('UrlBuilder', () => {
  ...
  it('should build url', () => {
    ...
    const fullPath = builder.getFullPath();
    ...
    expect(fullPath).toEqual('yeah/1?test%20value=false&nicer=dicer');
  });
  ...
  it('should add path trimmed', () => {
    const actual = new UrlBuilder('https://domain.com')
      .addPathTrimmed('/yeah')
      .addPathsTrimmed(['/path/', '/name/'])
      .getUrl();

    expect(actual).toEqual('https://domain.com/yeah/path/name');
  });
});
